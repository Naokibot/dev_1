...
static int probe(D3D_DRIVER_TYPE type, const wchar_t* label) {
    ...
    if (FAILED(hr)) { ::wprintf(L"D3D11_%ls_UNAVAILABLE hr=0x%08lx\n",label,(unsigned long)hr); return 1; }
    ...
    if(SUCCEEDED(hr)) ::wprintf(L"D3D11_OK driver=%ls feature=0x%04x adapter=%ls vendor=0x%04x device=0x%04x vram=%llu\n",label,(unsigned)selected,desc.Description,desc.VendorId,desc.DeviceId,(unsigned long long)desc.DedicatedVideoMemory);
    if(ad)ad->Release(); if(dxgi)dxgi->Release(); if(ctx)ctx->Release(); if(dev)dev->Release(); return SUCCEEDED(hr)?0:2;
}
